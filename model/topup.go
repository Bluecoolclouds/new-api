...
func RechargeHeleket(tradeNo string, callerIp string) (err error) {
        if tradeNo == "" {
                return errors.New("未提供支付单号")
        }
        ...
        err = DB.Transaction(func(tx *gorm.DB) error {
                ...
                if topUp.PaymentProvider != PaymentProviderHeleket {
                        return ErrPaymentMethodMismatch
                }
        ...
        if err != nil {
                common.SysError("heleket topup failed: " + err.Error())
                return errors.New("充值失败，请稍后重试")
        }
...
func RechargePally(tradeNo string, callerIp string) (err error) {
        if tradeNo == "" {
                return errors.New("未提供支付单号")
        }
        ...
        err = DB.Transaction(func(tx *gorm.DB) error {
                ...
                if topUp.PaymentProvider != PaymentProviderPally {
                        return ErrPaymentMethodMismatch
                }
        ...
        if err != nil {
                common.SysError("pally topup failed: " + err.Error())
                return errors.New("充值失败，请稍后重试")
        }
...
func RechargePlategal(tradeNo string, callerIp string) (err error) {
        if tradeNo == "" {
                return errors.New("未提供支付单号")
        }
        ...
        err = DB.Transaction(func(tx *gorm.DB) error {
                ...
                if topUp.PaymentProvider != PaymentProviderPlategal {
                        return ErrPaymentMethodMismatch
                }
        ...
        if err != nil {
                common.SysError("plategal topup failed: " + err.Error())
                return errors.New("充值失败，请稍后重试")
        }

        if quotaToAdd > 0 {
                RecordTopupLog(topUp.UserId, fmt.Sprintf("Platega充值成功，充值额度: %v，支付金额: %.2f RUB", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodPlategal)
                RewardReferralChain(topUp.UserId, quotaToAdd)
        }
