...
func getChannelQuery(group string, model string, retry int, ignoreChannelIds ...int) (*gorm.DB, error) {
	...
	if len(ignoreChannelIds) > 0 {
		channelQuery = channelQuery.Where("channel_id NOT IN (?)", ignoreChannelIds)
	}
	return channelQuery, nil
}

func GetChannel(group string, model string, retry int, requestPath string, ignoreChannelIds ...int) (*Channel, error) {
	var abilities []Ability

	var err error = nil
	channelQuery, err := getChannelQuery(group, model, retry, ignoreChannelIds...)
	if err != nil {
		return nil, err
	}
