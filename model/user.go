...
type User struct {
        Id               int            `json:"id"`
        Username         string         `json:"username" gorm:"unique;index" validate:"min=3,max=20"`
        Password         string         `json:"password" gorm:"not null;" validate:"min=8,max=72"`
...
func RewardReferralChain(userId int, quotaAdded int) {
        if quotaAdded <= 0 {
                return
        }
        rates := []float64{0.10, 0.05, 0.03}
        currentId := userId
        for level, rate := range rates {
                user, err := GetUserById(currentId, false)
                if err != nil || user == nil || user.InviterId == 0 {
                        break
                }
                inviterId := user.InviterId
                reward := int(float64(quotaAdded) * rate)
                if reward > 0 {
                        err = DB.Model(&User{}).Where("id = ?", inviterId).Updates(map[string]interface{}{
                                "aff_quota":   gorm.Expr("aff_quota + ?", reward),
                                "aff_history": gorm.Expr("aff_history + ?", reward),
                        }).Error
                        if err != nil {
                                common.SysError(fmt.Sprintf("referral reward level %d failed for user %d: %v", level+1, inviterId, err))
                        } else {
                                RecordLog(inviterId, LogTypeSystem, fmt.Sprintf("Referral reward level %d: +%s", level+1, logger.LogQuota(reward)))
                        }
                }
                currentId = inviterId
        }
...
func (user *User) Insert(inviterId int) error {
        ...
        if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() {
                if common.QuotaForInvitee > 0 {
                        ...
                        RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("Referral code bonus: %s", logger.LogQuota(common.QuotaForInvitee)))
                }
                if common.QuotaForInviter > 0 {
                        //_ = IncreaseUserQuota(inviterId, common.QuotaForInviter)
                        RecordLog(inviterId, LogTypeSystem, fmt.Sprintf("Referral reward: %s", logger.LogQuota(common.QuotaForInviter)))
