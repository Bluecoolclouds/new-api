...
// MaxTokensCapper is an optional interface adaptors can implement to enforce
// an upstream hard limit on max_tokens. The cap is applied AFTER param_override
// so it cannot be accidentally overridden by channel settings.
type MaxTokensCapper interface {
        MaxTokensCap() uint
}
