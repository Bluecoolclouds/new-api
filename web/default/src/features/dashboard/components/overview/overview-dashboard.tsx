...
function normalizeEndpoint(sourceUrl?: string): string {
  ...
  if (withoutTrailingSlash.endsWith('/v1/chat/completions')) return withoutTrailingSlash
...
function QuickActionItem(props: { action: QuickAction }) {
  ...
  return (
    <Button
      variant='outline'
      className='h-auto w-full justify-start overflow-hidden rounded-xl px-3 py-3 text-left whitespace-normal'
  ...
  const quotaQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await getUserQuotaDates(
        { start_timestamp: range.start_timestamp, end_timestamp: range.end_timestamp },
        false
      )
      return res?.data ?? []
    },
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div className='grid grid-cols-1 items-start gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]'>
        ...
        <div className='flex flex-col gap-4 min-w-0'>
          ...
          {/* 5. Recommended actions (quick actions grid) */}
          <CardStaggerContainer>
          ...
          {/* 7. Content panels (info / FAQ / uptime) */}
          {showContentPanels && (
            <CardStaggerContainer
              className={cn(
