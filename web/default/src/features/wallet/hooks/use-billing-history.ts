...
export function useBillingHistory(options: UseBillingHistoryOptions = {}) {
  ...
  const fetchBillingHistory = useCallback(async () => {
    ...
    try {
      const response = isAdmin
        ? await getAllBillingHistory(page, pageSize, keyword, paymentMethod)
  ...
  }, [isAdmin, page, pageSize, keyword, paymentMethod])
  ...
  /**
   * Filter by payment method (admin only)
   */
  const handlePaymentMethodFilter = useCallback((method: string) => {
